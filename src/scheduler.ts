...
const CHECK_INS: readonly CheckIn[] = [
  ...
  },
  {
    ...
    systemNote:
      ...
      `If lunch is already logged, reply with exactly the string ${SILENT} and call no tools. Do not add any text after ${SILENT}.`,
  },
  {
    cronExpr: "0 19 * * *",
    label: "DINNER",
    systemNote:
      ...
      `If dinner is already logged, reply with exactly the string ${SILENT} and call no tools. Do not add any text after ${SILENT}.`,
  },
  {
    cronExpr: "30 21 * * *",
    label: "EVENING",
    systemNote:
      ...
      "Step 1: Scan today's log for an existing Summary entry. If none is present, write a short summary into today's log via write_file, using the convention in protocol.md. DO NOT overwrite a Summary that is already there. " +
